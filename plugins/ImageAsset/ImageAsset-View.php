...
<script>
	...
	$(document).ready(function() {
		...
		$(".selector.image").change(function() {
			switch($(this).val()) {
				case "bundle":
					$(".image .complete").hide();
					$(".image .error").hide();
					$(".image .file").fadeIn("fast");
					$(".image .url").hide();
					$(".image .security").slideDown("fast");
					break;
				...
				case "library":
					$(".image .file").hide();
					$(".image .url").hide();
					$(".image .complete").fadeIn("fast");
					$(".image .error").fadeOut("fast");
					$(".image .security").slideUp("fast");
					break;
			}
		});
		...
		$(".upload").each(function() {
			...
			var test = new AjaxUpload(this, {
				...
				onSubmit: function(file, ext){
					...
					// set form value	
					if ($(button).hasClass("image")) $(button).parent().nextAll(':input[name="path"]').val(file);
		...
		// Display the "remove" icon for an image
		$(".security").live("mouseenter",function(){$(this).find(".expander").fadeIn("fast");});

		// Hide the "remove" icon for an image
...
<!-- Accordion Header -->
<h3 class="toggler">
	<div class="truncated artisthead">New Image Asset</div>
	<div class="truncated titlehead"></div>
	<div class="remove"><a href="#" onclick="removeImage($(this));"><img src="images/close_small.png"></a></div>
</h3>

<!-- Accordion Content => Image Editor -->
<div class="editor">
	<fieldset>
		<table class="input-form">
			...
			<tr class="uuid">
				<th>
					<label for="user_name">UID</label>
				</th>
				<td class="col-field">
					<input class="text" name="uuid" placeholder="Unique Identifier" value="" size="30" tabindex="3" type="text"/>
				</td>
			</tr>
			<tr class="format">
				<th>
				...
				<td class="col-field">
					<select class="dropdown" name="format" tabindex="5" type="text">
						<option disabled/>
						<option value="video/png">PNG</option>
						<option value="video/jpeg">JPG</option>
			...
			<tr class="image">
				...
				<td class="col-field">
					<table class="sub-table">
						<td class="sub-table-header">
							<select class="selector image" name="location" style="width: 100px;">
								<option disabled="disabled"></option>
								<option value="bundle">Bundle</option>
								<option value="remote">Remote</option>
							</select>
						...
						<td class="sub-table-col-field">
							<span class="url"><input class="form text url remote image" type="url" placeholder="http://" size="30" tabindex="6" /></span>
							<span class="file"><input class="upload file image" type="button" value="Choose File"/></span>
							<span class="processing"><img src="images/processing.gif" alt="Uploading file..."/></span>
							<span class="complete"><img src="images/complete.png" alt="File successfully uploaded"/></span>
							<span class="error"><img src="images/error.png" alt="Unable to upload file"/></span>
