...
<script>
	...
	$(document).ready(function() {
		...
		$(".selector.asset").change(function() {
			switch($(this).val()) {
				case "bundle":
					$(".asset .complete").hide();
					$(".asset .error").hide();
					$(".asset .file").fadeIn("fast");
					$(".asset .url").hide();
					$(".asset .security").slideDown("fast");
					break;
				...
				case "library":
					$(".asset .file").hide();
					$(".asset .url").hide();
					$(".asset .complete").fadeIn("fast");
					$(".asset .error").fadeOut("fast");
					$(".asset .security").slideUp("fast");
					break;
			}
		});
		...
		$(".upload").each(function() {
			...
			var test = new AjaxUpload(this, {
				...
				onSubmit: function(file, ext){
					...
					// set form value	
					if ($(button).hasClass("asset")) $(button).parent().nextAll(':input[name="path"]').val(file);
		...
		$(".asset.url").keyup(function(){
			$(".editor").find(':input[name="path"]').val($(this).val());
		});

		$(".timeToLiveDisplay").bind("input",function(){
			var ttl = $(this).val();

			if($(this).val() != "") {
				var ttl = "P"+$(this).val()+"D";
			}

			$(".security").find(':input[name="timeToLive"]').val(ttl);
		});
...
<!-- Accordion Header -->
<h3 class="toggler">
	<div class="truncated artisthead">New Audio Asset</div>
		...
		<table class="input-form">
			...
			<tr class="asset">
				...
				<td class="col-field">
					<table class="sub-table">
						<td class="sub-table-header">
							<select class="selector asset" name="location" style="width: 100px;">
								<option disabled="disabled"></option>
								<option value="bundle">Bundle</option>
								<option value="remote">Remote</option>
						...
						<td class="sub-table-col-field">
							<span class="url"><input class="form text url remote asset" type="url" placeholder="http://" size="30" tabindex="6" /></span>
							<span class="file"><input class="upload file asset" type="button" value="Choose File"/></span>
							<span class="processing"><img src="images/processing.gif" alt="Uploading file..."/></span>
							<span class="complete"><img src="images/complete.png" alt="File successfully uploaded"/></span>
							<span class="error"><img src="images/error.png" alt="Unable to upload file"/></span>
